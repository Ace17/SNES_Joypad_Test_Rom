...
void capture_inputs()
{
  ...
  for(i = 0; i < 4; i++) // super multitap not handled for now
  {
    ...
    if((getjoystatus(i) & UP_BUTTON) != 0)
    {
      writestring("UP", blockmap, posText + 0x00E, 0x3F6);
      setmap(0, (unsigned char*)blockmap);
      joypadHasInput = true;
    }
    if((getjoystatus(i) & DOWN_BUTTON) != 0)
    {
    ...
    if((getjoystatus(i) & START_BUTTON) != 0)
    {
    ...
    if((getjoystatus(i) & SELECT_BUTTON) != 0)
    {
